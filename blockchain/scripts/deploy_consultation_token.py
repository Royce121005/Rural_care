"""
Deploy ConsultationTokenVerifier smart contract to Sepolia testnet
"""
import os
import json
import sys
...
def deploy_contract():
    """Deploy the ConsultationTokenVerifier contract to Sepolia"""
    
    # Load environment variables
    load_dotenv()
    
    print("\n" + "=" * 80)
    print("DEPLOYING CONSULTATION TOKEN VERIFIER CONTRACT TO SEPOLIA")
    print("=" * 80)
    ...
    contract_path = Path(__file__).parent.parent / 'contracts' / 'ConsultationTokenVerifier.sol'
    
    with open(contract_path, 'r') as file:
        contract_source_code = file.read()
    
    print(f"✓ Contract source loaded: ConsultationTokenVerifier.sol")
    ...
    compiled_sol = compile_standard(
        {
            "language": "Solidity",
            "sources": {"ConsultationTokenVerifier.sol": {"content": contract_source_code}},
            "settings": {
                "outputSelection": {
                    "*": {
    ...
    contract_interface = compiled_sol['contracts']['ConsultationTokenVerifier.sol']['ConsultationTokenVerifier']
    bytecode = contract_interface['evm']['bytecode']['object']
    abi = contract_interface['abi']
    
    # Save ABI to file
    abi_path = Path(__file__).parent.parent / 'consultation_token_abi.json'
    with open(abi_path, 'w') as f:
        json.dump(abi, f, indent=2)
    print(f"✓ ABI saved to: {abi_path}")
    
    # Create contract instance
    ConsultationTokenVerifier = w3.eth.contract(abi=abi, bytecode=bytecode)
    ...
    transaction = ConsultationTokenVerifier.constructor().build_transaction({
    ...
    try:
        ...
        if tx_receipt.status == 1:
            ...
            print("=" * 80)
            print(f"\n📋 Contract Name: ConsultationTokenVerifier")
            ...
            deployment_info = {
                'contract_name': 'ConsultationTokenVerifier',
            ...
            deployment_path = Path(__file__).parent.parent / 'consultation_token_deployment_info.json'
            ...
            print(f"   CONSULTATION_TOKEN_CONTRACT_ADDRESS={contract_address}")
            print(f"\n2. Restart your Django server to load the new contract")
            print(f"\n3. Test in-person consultation token generation and blockchain verification")
            print("\n" + "=" * 80)
